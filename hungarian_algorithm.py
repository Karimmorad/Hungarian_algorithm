...
# takes as input the matrix m
# return the lines that covers the most zero
# this is the minimum number of lines that covers all the zeros
# in the adjacent cost matrix, the cost is the weight of the bipartite graph
def step3(m):
    dim = m.shape[0]
    assigned = np.array([])
        ...
        for j in range(0, dim):
            if m[i, j] == 0:
                if (np.sum(assignments[:, j]) == 0
                        and np.sum(assignments[i, :]) == 0):
                    assignments[i, j] = 1
                    assigned = np.append(assigned, i)
                else:
                    assignments[i, j] = 1
                    np.concatenate(([i], assigned))
    ...
    while len(new_marked_rows) > 0:
        ...
        for nr in new_marked_rows:
            zeros_col = np.argwhere(m[nr, :] == 0).reshape(-1)
            new_marked_cols = np.append(new_marked_cols, np.setdiff1d(zeros_col, marked_cols))
...
def final_assignment(initial_matrix, m):
    ...
    while np.sum(m == 0) > 0:
        ...
        m[i, j] += 1
        m[:, j] += 1
        assignment_single_zero_lines(m, assignment)
